...
def inject_theme():
    st.markdown(
        ...
        /* The dialog otherwise inherits config.toml's backgroundColor directly
           ...
           dark navy. [data-testid="stDialog"] is only the OUTER positioning
           wrapper though — the actual colored panel is an unnamed inner div a
           couple of levels down, so all of these are targeted to make sure
           the real one is caught regardless of exact nesting. */
