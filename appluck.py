    ...
    with left:
        if not st.session_state.loaded:
            ...
            if uploaded_file is not None:
                try:
                    file_ids = parse_ids_from_file(uploaded_file)
                except Exception as e:
                    st.error(f"Couldn't read that file — try re-saving it as .csv or .xlsx. ({e})")
            text_ids = parse_ids(text)
            combined_ids = list(dict.fromkeys(file_ids + text_ids))  # dedupe, keep order
            entry_count = len(combined_ids)
            ...
            with b1:
                if st.button("Load Participant", key="load_btn", disabled=entry_count == 0, use_container_width=True):
                    load_participants(combined_ids)
                    st.rerun()
            with b2:
                def _clear():
            ...
            with st.expander(f"View all {len(st.session_state.all_ids)} loaded IDs"):
                render_id_list(st.session_state.all_ids)
            status_panel()
            st.divider()
